...
function Cart() {
  ...
  const getAllCart = async () => {
    try {
      const { data } = await axios.get(`http://localhost:8080/api/cart/${auth.user._id}`);
      setCartProduct(data);
    } catch (error) {
  ...
  }, [auth.user, setCartProduct]);

  const removeCartItem = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/api/cart/${auth.user._id}/${id}`);
      getAllCart();
    } catch (error) {
      console.error("Cannot fetch data", error);
    }
